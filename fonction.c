...
World* CreateWorld(int NumberOfRoom){
    ...
    for(int i= 0; i < world->size.x; i++) {
        ...
        for(int j = 0; j < world->size.y; j++) world->map[i][j] = 0;
    }
...
void roomCreationInGame(Player *P1, World* world) {
    ...
    world->rooms[P1->roomIndex]->TabConnectedDoor[CurrentDoorIndex] = &(NextRoom->TabDoor[NextDoorIndex]);
    NextRoom->TabConnectedDoor[NextDoorIndex] = &(world->rooms[P1->roomIndex]->TabDoor[CurrentDoorIndex]);

    NextRoom->position.x = P1->Position.x - world->rooms[P1->roomIndex]->TabConnectedDoor[CurrentDoorIndex]->position.x;
    NextRoom->position.y = P1->Position.y - world->rooms[P1->roomIndex]->TabConnectedDoor[CurrentDoorIndex]->position.y;
...
void Travel(Player* P1, World* world){
   ...
   choice--;
    if (world->map[P1->Position.x  + MovX[choice]][P1->Position.y  + MovY[choice]] == 30+choice){
        if(world->rooms[P1->roomIndex]->TabConnectedDoor[choice]==NULL) {
            doorInteraction(P1, world,choice);
        }
        else{
            P1->Position.x += 3*MovX[choice];
            P1->Position.y += 3*MovY[choice];
            world->rooms[P1->roomIndex] = world->rooms[world->rooms[P1->roomIndex]->TabConnectedDoor[choice]->RoomIndex];
        }
    }
